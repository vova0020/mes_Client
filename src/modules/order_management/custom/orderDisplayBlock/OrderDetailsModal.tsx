...
import type { CustomOrderDetailedStatistics, CustomOrderPart } from '../../../api/custom';
import { customOrderStatisticsApi } from '../../../api/custom';

export type PalletStageStatus = 'NOT_STARTED' | 'IN_PROGRESS' | 'COMPLETED';

export interface CustomOrderPartExtended extends CustomOrderPart {
  partId?: number;
  packages?: string[];
}

export interface OrderDetailedStatistic extends Omit<CustomOrderDetailedStatistics, 'parts'> {
  packages?: any[];
  parts: CustomOrderPartExtended[];
}
...
const OrderDetailsModal: React.FC<OrderDetailsModalProps> = ({ orderId, orderDetails, onClose }) => {
  ...
  const handleConfirmClick = async () => {
    ...
    if (newCount >= 3) {
      ...
      try {
        await customOrderStatisticsApi.forceCompleteOrder(Number(orderId));
  ...
  const isPartRelatedToPackage = (partId: number | undefined, packageId: number | null) => {
    if (!packageId || !partId) return false;
        ...
        <div className={`${styles.modalContent} ${isVisible ? styles.slideIn : ''}`} onClick={(e) => e.stopPropagation()}>
          <div className={styles.modalHeader}>
            <div className={styles.headerLeft}>
              <h3>Детали заказа {orderDetails.batchNumber} (Индивидуальное)</h3>
              {/* Кнопка принудительного закрытия скрыта для индивидуального производства */}
            </div>
            ...
            <div className={styles.packagesSection}>
              ...
              <div className={styles.packagesTable}>
                <table>
                  ...
                  <tbody>
                    {orderDetails.packages?.map((pkg: any) => (
