...
import { useCustomOrderStatistics, useCustomOrderDetailedStatistics } from '../hooks';
import type { CustomOrderStatistics } from '../../../api/custom';

type OrderStatus = 'PRELIMINARY' | 'APPROVED' | 'LAUNCH_PERMITTED' | 'IN_PROGRESS' | 'COMPLETED' | 'POSTPONED';
...
const OrderDisplay: React.FC = () => {
  ...
  const { orders, loading, error } = useCustomOrderStatistics();
  const { orderDetails } = useCustomOrderDetailedStatistics(selectedOrderId);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h2 className={styles.title}>Отображение заказов (Индивидуальное производство)</h2>
      </div>
      ...
      <div className={styles.filtersContainer}>
        ...
        <div className={styles.filterRow}>
          ...
          {/* Упаковка скрыта для индивидуального производства */}
        </div>
      </div>
              ...
              <tr key={order.orderId} className={styles.tableRow}>
                ...
                <td className={styles.cell}>
                  <button
                    ...
                    onClick={() => handleShowDetails(order.orderId || order.customOrderId)}
                  >
                    Подробности
                  </button>
                </td>
